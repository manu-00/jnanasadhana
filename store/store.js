...
myApp.controller('storeCtrl', ['$scope', 'appService', function ($scope, appService) {
  ...
  function getallCategory() {
    $scope.cats = [];
    appService.getallCategory($scope.pageIndexC, $scope.pageSizeC).then(function (response) {
  ...
  $scope.deleteCat = function () {
    ...
    swal({
      ...
      .then((willDelete) => {
        if (willDelete) {
          appService.deleteCat($scope.cat).then(function (response) {
            $scope.pageChangeC();
            swal("deleted!", {
              icon: "success",
            });
  ...
  $scope.addCat = function () {
    appService.addCat($scope.cat).then(function (response) {
      $scope.pageChangeC();
  ...
  function getallMagazine() {
    $scope.books = [];
    appService.getallMagazine($scope.pageIndexB, $scope.pageSizeB).then(function (response) {
      angular.forEach(response.data.content, function (value) {
        $scope.books.push({
  ...
  $scope.deletebooks = function () {
    ...
    swal({
      ...
      .then((willDelete) => {
        if (willDelete) {
          appService.deletebooks($scope.book).then(function (response) {
            $scope.pageChangeB();
            swal("deleted!", {
              icon: "success",
            });
  ...
  $scope.addbooks = function () {
    appService.addbooks($scope.book).then(function (response) {
      $scope.pageChangeB();
      if (response.data.responseCode == 201) {
        swal("Done!", "", "success");
      }
